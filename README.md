...
Move the image from the Example folder into the following directory:
ACA mapping\U-Net_Pytorch\predict\

Use the pre-trained model to segment the input image.
...
By default, the scale is 0.5. To obtain better results (at the cost of higher memory usage), set it to 1. Automatic mixed precision is recommended and available with the --amp flag.
...
Images and masks are black and white. 

You may use your own dataset if you ensure proper loading in utils/data_loading.py.
...
Run the MATLAB Script: Execute the main MATLAB program 'main.m'.

Adjust Parameters: Adjust the following parameters within the script's comments:

'armdx' and 'armdz': Define the size of the parameter calculation window.

'ske_layer': Defines the layering method for rapid skeletonization. It is recommended that this value does not exceed 32.

'loc': Determines the percentage of image information loss, e.g., 0.1 corresponds to no more than 10% information loss.

'win_th': The calculation window threshold used to determine the spatial/frequency domain calculation for Local Coverage (LC).

Output Values: The average computed features are represented by the following variables:
